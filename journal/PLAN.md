...
| **B8** | **everything that won, at the corrected share** — F1 + no oversampling + self-training at 2 % | **running** (queued after `combine3-2pct`). Predicted probe **0.790–0.815**, held-out species within 0.01 of it; **falsified as a composition if ≤ 0.7747**. |
| **B7** | **the two best shifted levers combined** (L5's config + pseudo-labels) | **running** (12363549) → triple chained. Predicted probe 0.765–0.790; **falsified as a composition if ≤ 0.7538**. |
