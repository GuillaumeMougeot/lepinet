...
def filter_df(df, remove_in=[], keep_in=[],  min_img_per_spc=0):
    """
    Parameters
    ----------
    ...
    if min_img_per_spc > 0:
        print(f"Selecting {min_img_per_spc} images per species.")
        df=df[(df
            .groupby('speciesKey')['speciesKey']
            .transform('count') >= min_img_per_spc)]
    
    print(f"Length of the filtered DataFrame: {len(df)}.")
    return df

def filter_family_df(df, keep_in=[]):
    """Only keep familyKey in `keep_in`
    """
    if len(keep_in)>0:
        df=df.copy()
        df = df[df['familyKey'].isin(keep_in)]
        print(f"Length of the family filtered DataFrame: {len(df)}.")
    return df
...
def gen_dls(
    ...
    # Else, preprocessed the DataFrame
        ...
        df=filter_df(df, remove_in=["0"], min_img_per_spc=min_img_per_spc)
        print("DataFrame filtered.")

        if isinstance(family_filter, list) and len(family_filter)>0:
            df=filter_family_df(df, keep_in=family_filter)
