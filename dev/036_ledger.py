...
Read-only: it opens files under data/ and writes nothing.
...
def _run_progress(run_dir, live_names=frozenset()):
    ...
    Status is derived, not recorded: a run is 'done' once dev/030 exports the `.pt` bundle (its
    last act); else 'running' if a trainer process is writing that model_name; else 'died'.
    When several directories share a live model_name (a relaunch after a failed attempt), only
    the newest can be the live one -- caller resolves that.
    """
    exported = any(run_dir.glob("*.pt"))
