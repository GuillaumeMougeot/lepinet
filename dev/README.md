...
| `064_flemming_groups.py` | Grouped (trap, night) splits of the trap set + leakage assertions. B3's prerequisite: the shifted benchmark *is* the adaptation data, so B3 needs a disjoint `probe` set and a held-out-species subset. Exits non-zero rather than writing a leaky split. |
| `063_balanced_softmax.py` | Balanced Softmax as a `Callback` (via `train(..., extra_cbs=)`). Verified numerically to equal logit adjustment at τ=1 — the method this project already rejected, re-run because the reason it lost no longer applies. Refuses to run on `marginal*` heads, whose log-prob outputs it would corrupt. |
