...
| 7 | **Unlabelled target data supplies *coverage of the input space*; its *class distribution* is a second knob, and that knob only works on a frozen trunk.** Coverage: 0.39 % of training buys 97 % of the gain, and trading 24.6 pt of label accuracy for 740 more species buys **+4.24 pt**. Balance: rebalancing the pseudo-label set is worth **+1.51 probe / +1.87 held-out** to a frozen classifier and **−0.71 / −3.62** to end-to-end training — it concentrates replication on the classes with fewest unique images, and only a frozen trunk stops the representation memorising them. | [iteration](journal/2026-08-08-self-training-does-not-iterate.md) |
