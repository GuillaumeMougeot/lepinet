...
| 7 | **What unlabelled target data supplies is *coverage of the input space* — not labels, not gradient.** Three results converge, the last a controlled one: 0.39 % of training buys 97 % of the gain; a second round with **more accurate** labels (99.84 %) loses 3.80 pt by covering 156 species instead of 346; and changing **only the selection gate** — trading 24.6 pt of label accuracy for 740 more species — buys **+4.24 pt**. Anything that trades coverage for label quality trades the wrong way. | [iteration](journal/2026-08-08-self-training-does-not-iterate.md) |
