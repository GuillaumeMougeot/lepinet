...
| 7 | **What unlabelled target data supplies is *coverage of the input space* — not labels, not gradient.** Two results converge on it: 0.39 % of training buys 97 % of the gain, and a *second* self-training round with **more accurate** labels (99.84 % vs 98.15 %) loses **3.80 pt** because a better labeller separates its confidences and a fixed quantile gate then keeps 156 species instead of 346. Anything that trades coverage for label quality trades the wrong way. | [iteration](journal/2026-08-08-self-training-does-not-iterate.md) |
